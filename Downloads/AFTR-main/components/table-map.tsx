...
import { getTableStatuses, type TableStatuses } from "@/lib/tables"
...
export function TableMap({ selectedTable, onSelectTable }: TableMapProps) {
  ...
  const getApprovedCount = (tableId: number) => {
    return approvedCounts[tableId.toString()] || 0
  }

  const isTableOccupied = (table: Table) => {
    const flagOccupied = statusFlags[table.id.toString()] === true
    const reachedMinCovers = getApprovedCount(table.id) >= table.minCovers
    return flagOccupied || reachedMinCovers
  }

  const getSectionColor = (section: string, table: Table) => {
    if (isTableOccupied(table)) {
      return "bg-gray-700/50 border-gray-600/60 hover:bg-gray-700/70"
    }
  ...
  const getSelectedColor = (section: string, table: Table) => {
    if (isTableOccupied(table)) {
                ...
                <div key={row} className="space-y-1">
                  ...
                  <div className="grid grid-cols-7 gap-3">
                    {Array.from({ length: 7 }).map((_, colIndex) => {
                      ...
                      const approvedCount = getApprovedCount(table.id)
                      return (
                        <motion.button
                          key={table.id}
                          type="button"
                          ...
                          className={cn(
                            ...
                            isSelected ? getSelectedColor(table.section, table) : getSectionColor(table.section, table),
                        ...
                        >
                          <div className="text-center">
                            ...
                            {!isOccupied && approvedCount > 0 && (
                              <div className="absolute bottom-1 right-1 rounded bg-orange-500/80 px-2 py-0.5 text-[9px] font-semibold text-black">
                                {approvedCount}/{table.minCovers}
                              </div>
                            )}
                          </div>
                        </motion.button>
                  ...
                  return (
                      ...
                      className={cn(
                        ...
                        isOccupied
                          ? isSelected
                            ...
                            : "border-gray-600/60 bg-gray-700/50 text-white hover:bg-gray-700/70"
                          : isSelected
                    ...
                    >
                      <div className="text-center">
                        ...
                        {!isOccupied && approvedCount > 0 && (
                          <div className="absolute bottom-1 right-1 rounded bg-orange-500/80 px-2 py-0.5 text-[8px] font-semibold text-black">
                            {approvedCount}/8
                          </div>
                        )}
                      </div>
